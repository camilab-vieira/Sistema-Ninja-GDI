...
BEGIN	
	...
	FROM TIME_FAZ_MISSAO 
	WHERE NUMERO_DO_TIME = NUM;
...
BEGIN	
	SELECT COUNT(*) INTO QT 
	FROM PESSOA WHERE CADASTRO = ANY (SELECT CADASTRO_NINJA_TREINADO 
									  FROM NINJA_TREINA_NINJA 
									  WHERE CADASTRO_NINJA_TREINADOR = ANY (SELECT CADASTRO FROM KAGE));
...
BEGIN	
	SELECT COUNT(*) INTO QT 
	FROM NINJA_PARTICIPA_TIME 
	WHERE DATA = DIA AND TIPO_IO = 1;
...
BEGIN	
	 SELECT MAX(NUMERO) INTO QT 
	 FROM(SELECT COUNT(H.ID) AS NUMERO , H.TIPO 
		  FROM NINJA_TEM_HABILIDADE NTH INNER JOIN HABILIDADE H ON NTH.ID = H.ID 
		  WHERE NTH.CADASTRO = ANY(SELECT CADASTRO 
								   FROM NINJA_PARTICIPA_TIME 
								   WHERE NUMERO_DO_TIME = NUM) 
...
-- QUANTIDADE DE MISSOES SOLICITADAS NO DIA X
CREATE OR REPLACE PROCEDURE QT_SOLICITACAO_MISSAO_DATA(DIA IN DATE) IS QT NUMBER;
BEGIN	
	SELECT COUNT(*) INTO QT 
	FROM MISSAO 
	WHERE DATA_SOLICITACAO = DIA;
	DBMS_OUTPUT.PUT_LINE('QUANTIDADE DE MISSOES SOLICITADAS NA DATA ' || DIA ||': '||QT);
END;
...
BEGIN
    SELECT MAX(DIFICULDADE) INTO RANKING 
	FROM(SELECT COUNT(DIFICULDADE), DIFICULDADE 
		FROM (MISSAO M INNER JOIN TIME_FAZ_MISSAO TFM ON M.CODIGO = TFM.CODIGO) 
		WHERE TFM.NUMERO_DO_TIME = NUM GROUP BY DIFICULDADE);
RETURN RANKING;
END;
...
BEGIN
    SELECT EXTRACT(YEAR FROM MIN(NPT.DATA)) - EXTRACT(YEAR FROM MIN(P.DATA_NAS)) INTO IDADE 
	FROM NINJA_PARTICIPA_TIME NPT INNER JOIN PESSOA P ON NPT.CADASTRO = P.CADASTRO  
	WHERE NPT.TIPO_IO = 1 AND NPT.CADASTRO = CAD;
	RETURN IDADE;
END;
...
BEGIN
    SELECT DATA_NAS INTO NASC 
	FROM PESSOA 
	WHERE CADASTRO = (SELECT MIN(CADASTRO) 
					  FROM MISSAO 
					  WHERE DATA_SOLICITACAO = DATA);
	RETURN NASC;
END;
...
-- NOME DO PAIS GOVERNADO PELO KAGE X
	...
	BEGIN
	SELECT P.NOME INTO NOME_PAIS 
	FROM PAIS P INNER JOIN KAGE K ON P.ID = K.ID_VILA 
	WHERE K.CADASTRO = (SELECT CADASTRO 
						FROM PESSOA 
						WHERE NOME = LIDER);
	RETURN NOME_PAIS;
	END;